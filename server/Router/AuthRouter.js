...
router.post('/AddEvent', upload.none(), async (req, res) => {
    try {
        const { EventName, EventDescription, EventDate, EventTime, EventPrice, Completed } = req.body
        const newEvent = new EventDetails({ EventName, EventDescription, EventDate, EventTime, EventPrice, Completed })
        newEvent.save()
...
router.put('/EditEventDeatils/:id', upload.none(), async (req, res) => {
    try {
        ...
        const { EventName, EventDescription, EventDate, EventTime, EventPrice, Completed } = req.body
        const EditEvent = await EventDetails.findByIdAndUpdate(EventId,{EventName, EventDescription, EventDate, EventTime, EventPrice, Completed})
...
router.post('/EventUserRegistraion', async (req, res) => {
    try {
        console.log(req.body);
        const { TicketId, CustomerName, MobileNo, Email, Events, PaymentStatus, NumberOfMember, TransactionID, TicketPerPerson, TotalPayment, Dates, Claimed } = req.body
        const newuser = new EventUserRegistrations({ TicketId, CustomerName, MobileNo, PaymentStatus, Email, Events, NumberOfMember, TransactionID, TicketPerPerson, Claimed, TotalPayment, Date: Dates })
        newuser.save()
        res.json('newuser')
    } catch (error) {
        console.log(error);
...
router.put('/EventTicketClaimed/:id', async (req, res) => {
    try {
        TicketId = req.params.id
        console.log(TicketId, 'Claimed');
        const Claimeduser = await EventUserRegistrations.findByIdAndUpdate(TicketId, { Claimed: 'Claimed' })
        res.status(200).json(Claimeduser)
    } catch (error) {
        console.log(error);
        res.status(500).send(error)
...
router.put('/EventTicketCancelation/:id', async (req, res) => {
    try {
        TicketId = req.params.id
        const TicketCancel = await EventUserRegistrations.findByIdAndUpdate(TicketId, { Claimed: 'Canceled' })
        console.log(TicketCancel);
        res.status(200).json(TicketCancel)
    } catch (error) {
        res.status(500).send(error)
